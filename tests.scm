...
(define (list-partitions total max-pieces max-value)
    (define (recurse total max-pieces max-value partitions found)
        (cond((< max-pieces 0) partitions)
            ((= total 0)(cons found partitions))
            ((<= max-value 0) partitions)
            (else (append (recurse (- total max-value)(- max-pieces 1)
                max-value partitions (append found (list max-value)))
                (recurse total max-pieces (- max-value 1) partitions found)))))
    (recurse total max-pieces max-value '() '()))

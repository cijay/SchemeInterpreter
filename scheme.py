...
class Frame(object):
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('number of argument recieve different')
...
def do_lambda_form(vals, env):
    ...
    if len(vals.second) > 1:
        func_body = Pair('begin', vals.second)
    else:
        func_body = vals.second[0]
    return LambdaProcedure(formals, func_body, env)
...
def do_mu_form(vals):
    ...
    if len(vals.second) > 1:
        ...
        func_body = vals.second[0]
    return MuProcedure(formals, func_body)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        pair = Pair(target.second, vals.second)
        lambda_exp = do_lambda_form(pair, env)
        env.define(target[0], lambda_exp)
        return target[0]
    else:
...
def do_let_form(vals, env):
    ...
    for bind in bindings:
        ...
        duo = Pair(bind.second.first, values)
        values = Pair(scheme_eval(duo.first, env), values)
...
def check_formals(formals):
    ...
    if not scheme_listp(formals):
        raise SchemeError('Invalid list of symbols')
    parameters = set()
    for symbo in formals:
        if symbo in parameters:
            raise SchemeError('A symbol is repeated')
        elif not scheme_symbolp(symbo):
    ...
    while True:
        ...
        if (scheme_symbolp(first) # first might be unhashable
            ...
            if isinstance(procedure, PrimitiveProcedure):
                ...
                env, expr = procedure.env.make_call_frame(procedure.formals,\
                    args), procedure.body
            elif isinstance(procedure, MuProcedure):
                env, expr = env.make_call_frame(procedure.formals,args),\
                    procedure.body
            else:
                return scheme_apply(procedure, args, env)
